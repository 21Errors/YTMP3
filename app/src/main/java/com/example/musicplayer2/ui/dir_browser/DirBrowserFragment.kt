...
import com.example.musicplayer2.ui.dir_browser.FolderHolder.FolderCallBack
import com.example.musicplayer2.utils.PathUtils.getPathDown
import com.example.musicplayer2.utils.PathUtils.getPathUp
import com.example.musicplayer2.utils.PathUtils.isSubDir
...
class DirBrowserFragment : Fragment() {
        ...
        adapter.setListeners(object : AdapterListeners {
            override fun onCheckBoxClicked(holder: FolderHolder) {
                ...
                if (selectedCheckBox.isChecked) {
                    savedPaths!!.add(folder!!.absolutePath)
                } else {
                    savedPaths!!.remove(folder?.absolutePath)
                }
            ...
            override fun onItemClicked(holder: FolderHolder) {
                ...
                if (!folder!!.canRead() || subDirectories == null) {
                    Toast.makeText(
                        ...
                        getString(R.string.dir_browser_folder_unreadable),
                        Toast.LENGTH_SHORT
                    ).show()
                    return
                }
    ...
    fun onBackPressed() {
        if (isAtRoot) {
            ...
            symPathTextView!!.text = getPathUp((symPathTextView!!.text as String))
            currentAbsolutePath = getPathUp(currentAbsolutePath!!)
            isAtRoot = symPathTextView!!.text == "/"
            val current = File(currentAbsolutePath!!)
            val subDirectories: Array<File?> = (if (isAtRoot) rootDirs else current.listFiles()) ?: return
