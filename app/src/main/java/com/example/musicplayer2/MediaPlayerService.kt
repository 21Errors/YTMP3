...
import com.example.musicplayer2.MediaPlayerUtil.stop
...
class MediaPlayerService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        Timber.d("MediaPlayerService received intent with action: $action")

        // FIXED: Handle ACTION_STOP first, before any notification logic
        if (action == ACTION_STOP) {
            Timber.i("Sleep timer triggered: stopping playback")
            try {
                MediaPlayerUtil.stop()
                stopForeground(true) // Remove notification
                stopSelf() // Stop service
                Timber.i("Successfully stopped playback and service")
            } catch (e: Exception) {
                Timber.e(e, "Error stopping playback")
                stopSelf() // Stop service anyway
            }
            return START_NOT_STICKY
        }

        // FIXED: Handle fade out action
        if (action == ACTION_FADE_OUT_STOP) {
            Timber.i("Sleep timer triggered: fading out and stopping playback")
            try {
                MediaPlayerUtil.fadeOutAndStop(3000L) { // 3-second fade
                    Timber.i("Fade out completed, stopping service")
                    stopForeground(true) // Remove notification
                    stopSelf() // Stop service
                }
                Timber.i("Successfully started fade out")
            } catch (e: Exception) {
                Timber.e(e, "Error starting fade out, falling back to immediate stop")
                MediaPlayerUtil.stop()
                stopForeground(true)
                stopSelf()
            }
        ...
        // FIXED: Handle ACTION_STOP (immediate stop)
        if (action == ACTION_STOP) {
            Timber.i("Sleep timer triggered: stopping playback immediately")
            try {
                MediaPlayerUtil.stop()
                stopForeground(true) // Remove notification
                stopSelf() // Stop service
        ...
        // FIXED: Only create notification if we have a song to play
        if (intent?.hasExtra(EXTRA_SONG) == true) {
            // existing code for notifications + song handling
    ...
    fun refreshNotification() {
        ...
        // FIXED: Refresh foreground service with proper type
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                startForeground(
                    NOTIFICATION_ID,
            ...
            Timber.e(e, "Failed to refresh foreground service")
        }
    }
    ...
    companion object {
        ...
        const val ACTION_FADE_OUT_STOP ="fade out stop"
        ...
        const val EXTRA_SONG = "com.example.musicplayer2.song"
